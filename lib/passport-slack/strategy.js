...
function Strategy(options, verify) {
  ...
  this.identifyUrl = options.identifyUrl || "https://slack.com/api/auth.test?token="; // requires 'identity.basic' scope
  ...
  if(!this._skipUserProfile && this._scope.indexOf('identity.basic') === -1 && this._scope.indexOf('identify') === -1){
    console.warn("Scope 'identity.basic' or 'identify' is required to retrieve Slack user profile");
  }
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  if (this._scope.indexOf('identity.basic') >= 0) {
  ...
  } else if (this._scope.indexOf('identify') >= 0) {

    this.get(this.identifyUrl + accessToken, function (err, body, res) {
      if (err) {
        return done(err);
      } else {
        try {
